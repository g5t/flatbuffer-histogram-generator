...
inline const AlarmStatus (&EnumValuesAlarmStatus())[23] {
  static const AlarmStatus values[] = {
    ...
    AlarmStatus::WRITE_ACCESS,
    AlarmStatus::NO_CHANGE
  };
  return values;
}
...
inline const char *EnumNameAlarmStatus(AlarmStatus e) {
  if (::flatbuffers::IsOutRange(e, AlarmStatus::NO_ALARM, AlarmStatus::NO_CHANGE)) return "";
  const size_t index = static_cast<size_t>(e);
  return EnumNamesAlarmStatus()[index];
}

enum class AlarmSeverity : uint16_t {
  MINOR = 0,
  MAJOR = 1,
  NO_ALARM = 2,
  INVALID = 3,
...
inline const char *EnumNameAlarmSeverity(AlarmSeverity e) {
  if (::flatbuffers::IsOutRange(e, AlarmSeverity::MINOR, AlarmSeverity::NO_CHANGE)) return "";
  const size_t index = static_cast<size_t>(e);
...
struct LogData FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  bool mutate_status(AlarmStatus _status = static_cast<AlarmStatus>(22)) {
    return SetField<uint16_t>(VT_STATUS, static_cast<uint16_t>(_status), 22);
  }
  AlarmSeverity severity() const {
    return static_cast<AlarmSeverity>(GetField<uint16_t>(VT_SEVERITY, 4));
  }
  bool mutate_severity(AlarmSeverity _severity = static_cast<AlarmSeverity>(4)) {
    return SetField<uint16_t>(VT_SEVERITY, static_cast<uint16_t>(_severity), 4);
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
inline ::flatbuffers::Offset<LogData> CreateLogData(
    ...
    AlarmStatus status = AlarmStatus::NO_CHANGE,
    AlarmSeverity severity = AlarmSeverity::NO_CHANGE) {
...
inline LogData *GetMutableSizePrefixedLogData(void *buf) {
  return ::flatbuffers::GetMutableSizePrefixedRoot<LogData>(buf);
}
